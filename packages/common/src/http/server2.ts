...
export class Server {
	...
	/*
		...
		1) путь '/' - 503 если счетчик >= установленного (добавить в деплойментс лайвнесс)
		2) путь реаднесс - сервер из реди
		3) контекст накинуть, установить реквест айди, зарегистрировать
			- контекст установить
		4)


		в файле /Users/oleg/WebstormProjects/fw/packages/common/src/http/server2.ts
		нужно сделать хук onRequest в который не пойдут запросы OPTIONS, '/', '/_/readiness'.
		Если задан customCountryHeaders то взять первое непустое значение и записать в request.country.
		Если пусто то 'zz'.
