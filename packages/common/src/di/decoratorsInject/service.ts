...
export function UseService(typeFn?: (type?: never) => Constructable<unknown>): Func {
			...
			value: (containerInstance) => {
				...
				if (targetService?.instanceOf && !['action'].includes(targetService.instanceOf)) {
					throw new DIErrorInject(
						`${(target as Constructable<unknown>).constructor.name} -> ${propertyName.toString()} -> Services are allowed only in actions`,
					);
				}
				...
				return Registry.get(containerInstance, 'service', evaluatedLazyType);
			},
		});
	};
}
