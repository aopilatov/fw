...
export function UseCache(typeFn?: (type?: never) => Constructable<unknown>): Func {
			...
			value: (containerInstance) => {
				...
				if (targetService?.instanceOf && !['service'].includes(targetService.instanceOf)) {
					throw new DIErrorInject(
						`${(target as Constructable<unknown>).constructor.name} -> ${propertyName.toString()} -> Cache is allowed only in services`,
					);
				}
				...
				return Registry.get(containerInstance, 'cache', evaluatedLazyType);
			},
		});
	};
}
