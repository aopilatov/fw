...
export function UseWarehouse(typeFn?: (type?: never) => Constructable<unknown>): Func {
	return function (target: object, propertyName: string | symbol, index?: number): void {
		...
		Registry.registerHandler({
			...
			value: (containerInstance) => {
				...
				if (targetService?.instanceOf && !['service'].includes(targetService.instanceOf)) {
					throw new DIErrorInject(
						`${(target as Constructable<unknown>).constructor.name} -> ${propertyName.toString()} -> Warehouses is allowed only in services`,
					);
				}
				...
				return Registry.get(containerInstance, 'warehouse', evaluatedLazyType);
			},
		});
	};
}
