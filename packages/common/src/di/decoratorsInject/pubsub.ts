...
export function UsePubsub(typeFn?: (type?: never) => Constructable<unknown>): Func {
			...
			value: (containerInstance) => {
				...
				if (targetService?.instanceOf && !['action'].includes(targetService.instanceOf)) {
					throw new DIErrorInject(
						`${(target as Constructable<unknown>).constructor.name} -> ${propertyName.toString()} -> PubSubs are allowed only in actions`,
					);
				}
				...
				return Registry.get(containerInstance, 'pubsub', evaluatedLazyType);
			},
		});
	};
}
