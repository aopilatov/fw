			...
			value: (containerInstance) => {
				...
				if (targetService?.instanceOf && !['service'].includes(targetService.instanceOf)) {
					throw new DIErrorInject(
						`${(target as Constructable<unknown>).constructor.name} -> ${propertyName.toString()} -> Repository is allowed only in services`,
					);
				}
				...
				return Registry.get(containerInstance, 'repository', evaluatedLazyType);
			},
		});
	};
}
